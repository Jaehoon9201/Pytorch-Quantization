...
def print_size_of_model(model, label=""):
    # torch.save(model.state_dict(), "temp/temp_%s.pt" %(label))
    # # torch.save(model, "temp/temp_%s.pt" % (label))
    # size=os.path.getsize("temp/temp_%s.pt" %(label))
...
input_fp32 = torch.randn(1000, 1, 10)
...
confusion_mtrix_plot(labels_all, y_pred_all_int8, 'static_qat_int8')
